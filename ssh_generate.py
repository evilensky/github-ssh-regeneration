...
def main():
    ...
    projects_url = (
        "https://circleci.com/api/v1.1/organization/{vcs}/{org}/settings".format(
            vcs=VCS, org=ORG
        )
    )
    ...
    if projects_resp.error_count > 0:
        raise Exception(
            "error retrieving projects: {response} url: {url}".format(
                response=str(projects_resp), url=projects_url
            )
        )
    ...
    for project in project_names:
        ...
        if ssh_keys_resp.error_count > 0:
            raise Exception(
                "error getting ssh_keys: {response} url: {url}".format(
                    response=str(ssh_keys_resp), url=ssh_keys_url
                )
            )
        ...
        if key_info["preferred"] and key_info["public_key"].startswith("ssh-rsa"):
            ...
            print(
                "{project} old preffered key: old_keys_{org}.txt".format(
            ...
            print(
                "Creating key for {project} logging new key to new_keys_{org}.txt".format(
                    project=project, org=ORG
                )
            )
            ...
            checkout_key_url = "https://circleci.com/api/v2/project/{vcs}/{org}/{project}/checkout-key".format(
                vcs=VCS, project=project, org=ORG
            )
            ...
            if checkout_key_resp.status == 403 or checkout_key_resp.status == 500:
                ...
                raise Exception(
                    "error creating new checkout key: {response} url: {url}".format(
                        response=str(checkout_key_resp), url=checkout_key_url
                    )
                )
            ...
            new_keys_file.write(
                "{project} new prefered key: {key} \n".format(
                    project=project, key=str(checkout_key_resp)
                )
            )
            new_keys_file.close()
        else:
            print(
                "{project} prefered key is not ssh-rsa, no action taken.".format(
                    project=project
                )
            )
